...
```{r}
ggplot(turnout) + 
  geom_bar(aes(x=factor(voted1996))) + 
  facet_wrap(~GOTV_call) + labs(x="Whether or not someone voted in 1996, 0 = no, 1 = yes",y="Frequency",title="Proportions of Votes/No Votes in 1996, by GOTV call")
```

The code here shows a clear discrepancy between the counts as well as the proportions of those who received a GOTV_call and those who did not and whether or not they voted in 1996. The bars in those who did not receive a call are much closer in size while the opposite is true for the other bars, showing a difference in proportions.
...
Along with the plot, our 95% confidence interval from 0.12 to 0.24 for the difference in proportions of voters in 1996 on whether or not they received a GOTV_call does not contain the value zero, meaning the interval is statistically significant and supports the claim of there being a correlation here.
...
```{r}
ggplot(turnout) + 
  geom_bar(aes(x=factor(MAJORPTY))) + 
  facet_wrap(~GOTV_call) + labs(x="Whether or not registered for Major Party, 0 = no, 1 = yes",y="Frequency",title="Proportions of Major Party/Not Major Party by GOTV call")
```

Similar to the first barplot, this graph depicts the difference in proportions of those who were registered for a major party and did/did not receive a a GOTV_call. However, it is not completely easy to see a difference in these proportions. So, we can use a confidence interval.
...
To further support the table above, we can once again make our graphs but this time notice that the proportions of those who voted in 1996 for both facets are very similar, regardless of whether or not they received a GOTV_call.
...
Also notice how the 95% confidence interval that was previously statistically significant is now no longer statistically significant since it ranges from -0.07 to 0.06 which includes 0, meaning it does not support the claim that voting in 1996 could be a confounding variable.
...
Again, the proportions of those who registered for a major party and did/did not receive a GOTV_call are nearly identical with the 0 and 1 at the top separating the data based on the GOTV_call and the bottom representing whether or not someone was registered for a major party. This is evidence that they are no longer confounding variables in our matched data because they correlate with our treatment variable and outcome variable.
...
The 95% confidence interval to illustrate this difference ranged from -0.07 to 0.04 meaning it is not statistically significant, further supporting the claim that GOTV_call is not correlated to being registered for a major party in our matched data set.
...
To wrap this up, we can quite confidently state that the GOTV_call had essentially no effect on the likelihood/proportion of voting in the 1998 election.